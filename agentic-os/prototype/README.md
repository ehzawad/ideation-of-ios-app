...
**One file:** download [`the-line.html`](the-line.html) and open it in a browser. Everything (styles, the OS, the planners and the UI) is inside that single HTML file, so you can open it from disk, email it, or drop it on any static host. Nothing to install, no build step, and nothing touches your real device.

**From source:** `index.html` loads the same code from `os.js`, `planner.js` and `ui.js`, which is easier to read and edit. After changing them, regenerate the single file:

```sh
python3 scripts/bundle_prototype.py
```

Voice input uses the browser's speech recognition (Chrome, Edge and Safari have it). Replies to voice requests are spoken back. The live-model planner only works when the page runs as a claude.ai artifact; everywhere else the offline rules plan every request.
...
| `the-line.html` | The whole simulator in one self-contained file, generated by `scripts/bundle_prototype.py`. |
