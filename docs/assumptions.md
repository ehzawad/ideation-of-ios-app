...
| **"On-device means private, cloud means not."** | On-device models can still leak through what the agent *does* (sending a message to the wrong person). Apple's Private Cloud Compute is designed for privacy while running in the cloud. | Context leaking to the wrong recipient happens in most tests of frontier agents ([Agent CI Bench](https://huggingface.co/papers/2606.23189)). |
...
5. **How much can a 3B-parameter model on a phone really plan?** The on-device model is good at extraction and short tool calls. Long-horizon planning on-device is an open research question.
