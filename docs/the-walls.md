...
| **No cellular call audio** | No public API records, transcribes or screens cellular calls. Call Screening and Hold Assist (iOS 26) are Apple-only. Recording needs visible consent (guideline 2.5.14). | Place calls from the cloud (Twilio and similar), or own the audio path in a VoIP app with [CallKit](https://developer.apple.com/documentation/callkit). |
...
| **Small on-device model** | The on-device model has [a 4,096-token context window per session](https://developer.apple.com/documentation/foundationmodels/managing-the-context-window). Tool schemas, tool outputs and history all count. Model behavior changes with OS versions. | Split tasks across sessions, retrieve instead of stuffing context, run [Evaluations](https://developer.apple.com/documentation/foundationmodels) per OS version, escalate to Private Cloud Compute (32K tokens) or a cloud model. |
...
| **2.5.2** and **4.2.6** | Apps may not download or run code that adds or changes features. | An agent may fill in parameters for tools that are compiled into the app. It may not write and run new app logic. Apple blocked updates to vibe-coding apps under this rule in March 2026 (press reports: [MacRumors](https://www.macrumors.com/2026/03/18/apple-blocks-updates-for-vibe-coding-apps/)). |
...
| **5.1.1(ix)** | Apps in highly regulated fields (banking, health care, air travel, crypto) must be submitted by the legal entity that provides the service. | A finance or health agent may need a licensed partner as the publisher. |
...
- **Private Cloud Compute is free, with limits.** Apple's server model is free for [developers in the Small Business Program with fewer than 2 million first-time downloads](https://developer.apple.com/documentation/foundationmodels/adding-server-side-intelligence-with-private-cloud-compute). Each user has a daily quota. Cross the threshold and you have six months to move to another provider.
...
- **Japan (Mobile Software Competition Act):** since [iOS 26.2](https://developer.apple.com/news/?id=074b3wzz), users can put a third-party voice assistant on the side button.
...
- [Agent CI Bench](https://huggingface.co/papers/2606.23189): 12 of 15 frontier computer-use agents leaked personal context to the wrong recipient in more than half the scenarios.
