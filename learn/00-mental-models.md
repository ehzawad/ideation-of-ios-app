...
There are 35 mental models in 10 parts. Each is a bold sentence you could say out loud, the reason behind it, and a **senior tell**: how experienced developers act on it. Short on time? Read only the bold lines first (about 10 minutes), then come back.
...
| B · Declarative UI, I · Design | [Day 2](day2-swiftui-liquid-glass-design.md) |
| C · Swift | [Day 1](day1-swift-and-concurrency.md) |
| D · System surfaces | [Day 4](day4-app-intents-siri-system-surfaces.md) |
| E · Apple Intelligence, F · ML and GPU | [Day 5](day5-apple-intelligence-and-ml.md), [Day 6](day6-metal4-graphics-and-compute.md) |
...
Each app gets its own container on disk and can't read other apps' files or see their screens. App Review guideline 2.5.2 says apps "may not read or write data outside the designated container area." To reach anything else you go through a framework, and three kinds of keys open the doors:

- **Entitlements**: signed claims in your code signature for capabilities like iCloud or App Groups. Some are *managed*, meaning Apple must approve you first. The Private Cloud Compute entitlement, `com.apple.developer.private-cloud-compute` (iOS 27), is one.
- **Permissions**: the prompts people see. Each needs a purpose string in `Info.plist`, such as `NSCameraUsageDescription`. Without it, the app stops the moment it touches the resource.
- **Privacy manifests**: a `PrivacyInfo.xcprivacy` file listing the data you collect and why you call certain "required reason" APIs. Third-party SDKs ship their own.

People can also grant partial access, like a few selected photos. Guideline 5.1.1(iv) asks you to respect a "no" and offer another way where you can.
...
| 2.5.4 | Background modes only for their stated purpose. No silent audio to stay awake. |
...
| 5.1.2(i) | Get explicit permission before sharing personal data with third parties, "including with third-party AI." |

**Senior tell:** they read the relevant guideline before designing a feature and write the review notes while they build it.

**3. The OS owns your lifecycle: it launches, suspends and ends your app, one scene at a time.**

You don't decide when your code runs. The system launches your process, sometimes straight into the background to handle a push, a location event or an App Intent. When people leave, it moves you to the background and soon *suspends* you: still in memory, but with no CPU time. When memory runs short, it can end a suspended app. Apple's docs say UIKit "can disconnect a background or suspended scene at any time to reclaim its resources."
...
The *process* and the *UI* have separate lifecycles. The process launches once; each **scene** (one instance of your UI, like a window) has its own state. On iPad, one window of your app can be in the foreground while another is in the background. In iOS 27 this stopped being optional: apps built with the latest SDK "must adopt the scene-based life cycle or they fail to launch." In SwiftUI, `WindowGroup` gives you scenes, and the `scenePhase` environment value tells a view whether its scene is active, inactive or in the background.
...
Third-party apps get no always-on loop. Each kind of background work has its own tool and limits:

| You need to | Use | The deal |
|---|---|---|
| Refresh content now and then | `BGAppRefreshTask` | The system picks the time; up to 30 seconds. |
| Do heavy work later | `BGProcessingTask` | Runs when the system chooses, often on the charger. |
...
Energy sits behind all of these: the CPU, GPU, Neural Engine, radios and GPS drain the battery and heat the phone. Read `ProcessInfo.thermalState` and `isLowPowerModeEnabled` and do less when they say so. In iOS 27, background inference on the Neural Engine even needs its own entitlement.
...
A SwiftUI `View` is a struct with a `body`. When state that `body` reads changes, SwiftUI calls `body` again, compares the result with the last one, and updates only what changed on screen. View structs are created and discarded constantly, which is cheap because they're values. It's also why you don't do real work in a view's `init` or `body`: they can run many times a second.
...
SwiftUI must decide whether the view it sees now is the same view as last time. It uses *structural identity* (the view's position in the tree, such as one branch of an `if`) and *explicit identity* (the IDs you give `ForEach` or the `id(_:)` modifier). Same identity: state and running tasks survive. New identity: they're discarded. Apple's docs for `id(_:)`: when the value changes, "the identity of the view — for example, its state — is reset." A `.task(id:)` is cancelled and restarted when its id changes.
...
Each piece of mutable state has one owner, its **source of truth**. The owner passes plain values down to children that only read, and a `Binding` to children that write. Children send events back up through closures or binding writes. Shared services live in the environment.
...
The whole idea in one small view:
...
- `@State` makes `ErrandList` the owner. In Xcode 27, `@State` is a macro, and an object stored in it is created "the first time SwiftUI instantiates the view."
...
**Senior tell:** when two screens show different values for the same thing, they find the second source of truth and delete it.
...
SwiftUI is the default for new UI, and some surfaces exist only in SwiftUI: widgets, Live Activities, Controls and interactive snippets. UIKit is still underneath, still maintained, and still home to mature pieces like advanced text editing and very large collection layouts, plus years of existing code. The two work in both directions: `UIViewRepresentable` and `UIViewControllerRepresentable` put UIKit inside SwiftUI; `UIHostingController` and (iOS 26) `UIHostingSceneDelegate` put SwiftUI inside UIKit.
...
Structs and enums are *value types*: assigning one makes a copy, so nobody can change your copy behind your back. Arrays, dictionaries and strings are values too, and copy only when written. Classes are *reference types*: every variable points at the same object. Use a class when identity matters, such as an `@Observable` model that several screens watch or a UIKit view. Swift replaces most inheritance with **protocols**: `View`, `AppIntent`, `Tool` and `LanguageModel` are all protocols your types conform to, and generics let you write code against a protocol without a base class.
...
`@Observable`, `@Model` (SwiftData), `@Generable` (Foundation Models), `#Preview`, `@Test` and `#expect` (Swift Testing), and in Xcode 27 `@State` are all macros. A macro only *adds* code next to yours; it never modifies what you wrote. In Xcode, click a macro and choose **Editor > Expand Macro** to read the generated code. You can even set breakpoints in it.
...
In Swift 6 mode, strict concurrency checking is always complete and every finding is an error.

**Senior tell:** when the compiler complains, they fix who owns the state instead of silencing it with `@unchecked Sendable`.

**15. In an app target, start on the main actor and leave it on purpose.**

Most app code touches UI state, so Xcode has a build setting, **Default Actor Isolation** (`SWIFT_DEFAULT_ACTOR_ISOLATION`). Set it to `MainActor` and unannotated code is main-actor code, which removes most false alarms in ordinary sequential code. **Approachable Concurrency** (`SWIFT_APPROACHABLE_CONCURRENCY`) adds related features; one makes a `nonisolated async` function run on its caller's actor "unless the function is explicitly marked `@concurrent`." The rule becomes simple: code runs on the main actor unless you say otherwise. Shared state that must live elsewhere goes in an `actor`; CPU-heavy work goes in a `@concurrent` function.

**Senior tell:** every hop off the main actor has a name, an `actor` or a `@concurrent` function, so a reviewer can see it.
...
|---|---|---|
| Widgets | Glanceable views on the Home Screen and Lock Screen | WidgetKit and SwiftUI |
...
Most of these run as **extensions**: separate processes with tight memory and time limits, sharing data with your app through an App Group. A Live Activity, for example, stays active for at most 8 hours (12 on the Lock Screen), and its data can't exceed 4 KB.

**Senior tell:** they design the glance first (what fits in a widget or Live Activity) and the full screen second.

**17. App Intents are the verbs and nouns your app exports to the OS.**

An **app intent** is a verb: one action with typed parameters and a `perform()` method. An **app entity** is a noun: a lightweight, identifiable version of one of your data objects that the system can find, show and pass around. An **app enum** is a fixed set of choices. Apple calls intents "the gateway to your app's features." Write an intent once, and Siri, Shortcuts, Spotlight, interactive widgets, Controls, the Action button and your own buttons can all run it.
...
Siri AI, new in iOS 27, handles requests that span apps. It reaches third-party apps only through App Intents. **App schemas** are Apple-defined shapes for common actions and content, grouped in domains such as Mail, Calendar, Photos, Notes and Reminders. Conform an intent with `@AppIntent(schema:)` and, in Apple's words, you make it available "to the app toolbox, which Apple Intelligence draws on to service requests." Your app can't call other apps' intents; only the system orchestrates. At launch, Siri AI is an opt-in beta in limited regions and languages (see [the landscape](../docs/landscape-2026.md)).
...
**19. On-device first, Private Cloud Compute second, a third-party cloud third, and that one only with consent.**
...
**Senior tell:** they start every AI feature on the on-device model, measure it, and move up a rung only when an evaluation says they must.

**20. The model is part of the OS, and it changes when the OS updates.**

The on-device model ships inside iOS. Apple updated it in iOS 26.4 and again in iOS 27, both times telling developers to "test your prompts with the new model to verify your app's behavior." It can also be missing: `SystemLanguageModel.default.availability` reports `deviceNotEligible`, `appleIntelligenceNotEnabled` or `modelNotReady`. So every AI feature needs a path that works without it. The Evaluations framework (iOS 27) scores a feature against a fixed sample set, so a model change shows up as a number in your tests instead of in reviews.
...
With the `@Generable` macro you describe the output as a Swift type, and the framework uses *constrained sampling* so the model can only produce valid instances of it. `@Guide` adds a short description or a constraint, like a range or a count. No JSON to parse, no "please answer in this format."
...
- The model fills properties in declaration order, so declare first what it should work out first.
...
A **tool** conforms to `Tool`: a name, a description, typed arguments, and a `call(arguments:)` method. The model decides when to call it; your code does the real work, such as searching your store. `GenerationOptions.ToolCallingMode` (iOS 27) lets a request allow, require or forbid tool calls. The `LanguageModel` protocol (iOS 27) is the other half: `SystemLanguageModel` and `PrivateCloudComputeLanguageModel` conform, and so can a model you run with Core AI or another company's server model. Your session code, tools and `@Generable` types stay the same when the model changes.
...
Apple ships tuned models behind plain APIs, managed by the OS and adding nothing to your app's size: Vision (`RecognizeTextRequest`, `RecognizeDocumentsRequest`), Speech (`SpeechAnalyzer`, iOS 26), Translation (`TranslationSession`), Natural Language (`NLTagger`), and Foundation Models for language. Bring your own model only when the task is specific to your domain. For that, iOS 27 adds **Core AI**, which runs neural networks you convert to its `.aimodel` format across the CPU, GPU and Neural Engine, and can serve a language model to Foundation Models. **Core ML** remains for other model types, like decision trees and tabular pipelines. Metal sits under everything; Metal 4 can even run inference inside a GPU frame with `MTL4MachineLearningCommandEncoder`.
...
A SwiftUI shader is a Metal shader function that SwiftUI runs for you: GPU speed without owning a render loop.
...
Mobile networks drop. An app that saves on the device first and syncs later keeps working in a subway tunnel. Where things go:
...
| Large files, documents, caches | Files in your container, via `FileManager` |
...
Once your app has saved data on people's phones, every change to its shape needs a migration, and people skip versions: someone may jump from your v1 to v5 in one update. SwiftData handles this with `VersionedSchema` and `SchemaMigrationPlan`. CloudKit is stricter: once you promote a schema to production it's additive only, and you can't delete model types or change existing attributes.
...
TestFlight lets up to 100 internal testers and up to 10,000 external testers install a build, each build for up to 90 days; the first build for external testers goes through review. On the App Store, a **phased release** gives an update to automatic-update users over 7 days: 1%, 2%, 5%, 10%, 20%, 50%, then 100%. You can pause it for up to 30 days in total, and anyone can still download the update by hand.

**Senior tell:** during a phased release they check Organizer and crash reports daily and pause at the first unusual spike.

### I · Design

**31. Content comes first. Controls float on glass.**

**Liquid Glass** is the design material of iOS 26 and later. It forms "a distinct functional layer for controls and navigation elements" that floats above your content and lets it show through. Apple's stated goal is interfaces that "establish hierarchy, create harmony, and maintain consistency." Standard SwiftUI and UIKit components pick it up automatically when you build with the current Xcode. For custom controls there's `glassEffect(_:in:)`, `GlassEffectContainer` and the `.glass` button style. The HIG's rules: no glass in the content layer, use it sparingly, and remove custom bar backgrounds so they don't fight the system effect. In June 2026 the HIG also reintroduced its design principles: purpose, agency, responsibility, familiarity, flexibility, simplicity, craft and delight.
...
Use the built-in text styles and **Dynamic Type** comes with them, including the larger accessibility sizes. Use **SF Symbols** for icons; they sit well beside text and offer several rendering modes. Give every icon-only button an accessibility label for VoiceOver and Voice Control. People can also reduce transparency, increase contrast or reduce motion, and Liquid Glass adapts. On the App Store, **Accessibility Nutrition Labels** tell people which features you support, such as VoiceOver, Larger Text and Reduced Motion.
...
Every June at WWDC, Apple releases the first betas of the next OS and Xcode. Betas continue through the summer, APIs can still change, and the release comes in September. Point releases add features too. Removals are slow and loud. The scene lifecycle is the textbook case: iOS 18.4 logged a message, iOS 26 warned it "will soon be required," and iOS 27 won't launch apps without it. Xcode 27 runs only on Apple silicon Macs and deprecates On Demand Resources in favor of Background Assets.

**Senior tell:** they build with the new Xcode beta in June and clear every new warning before September.

**35. Apple's docs and sample code are the spec. The Updates pages are the changelog.**

When a blog post and Apple's documentation disagree, the documentation wins. When the documentation is thin, Apple's sample code shows the intended pattern: Landmarks for Liquid Glass, Origami for Foundation Models with Private Cloud Compute. The [Updates](https://developer.apple.com/documentation/updates) pages list each framework's changes by release, and together they show the direction: the system is becoming an agent (Siri AI) that calls apps as typed tools (App Intents); models run on device first and in Apple's private cloud second; layouts are content-first and adaptive; and spatial computing uses the same SwiftUI and RealityKit as iPhone.
...
The frameworks that matter in 2026. "Since" is the iOS version that introduced the framework.
...
| [UIKit](https://developer.apple.com/documentation/uikit) | Imperative UI; the layer under SwiftUI | A control SwiftUI lacks, huge lists, existing code | iOS 2 |
...
| **Data and accounts** | | | |
| [SwiftData](https://developer.apple.com/documentation/swiftdata) | Persist your model graph | Saving data on the device | iOS 17 |
| [CloudKit](https://developer.apple.com/documentation/cloudkit) | iCloud databases and sync | Sync across devices, sharing | iOS 8 |
| [Security](https://developer.apple.com/documentation/security) | Keychain storage for secrets | Tokens, passwords, keys | iOS 2 |
...
| [StoreKit](https://developer.apple.com/documentation/storekit) | In-app purchases, subscriptions | Selling digital goods | iOS 3 |
| **System surfaces** | | | |
| [App Intents](https://developer.apple.com/documentation/appintents) | Actions and content for Siri, Shortcuts, Spotlight, widgets | Anything people might do from outside your app | iOS 16 |
...
| [Core ML](https://developer.apple.com/documentation/coreml) | Other model types, existing `.mlmodel` files | Trees, tabular models, older pipelines | iOS 11 |
...
| [Translation](https://developer.apple.com/documentation/translation), [Natural Language](https://developer.apple.com/documentation/naturallanguage) | Translation; language detection and tagging | Multilingual content | iOS 17.4, iOS 12 |
...
| **Graphics and media** | | | |
| [Metal](https://developer.apple.com/documentation/metal) | GPU rendering and compute; `MTL4` types since iOS 26 | Custom renderers, compute, ML in the frame | iOS 8 |
| [RealityKit](https://developer.apple.com/documentation/realitykit) | 3D and AR | 3D objects, physics, AR | iOS 13 |
| [AVFoundation](https://developer.apple.com/documentation/avfoundation) | Camera, audio, video capture and playback | Media features | iOS 2.2 |
| **Quality** | | | |
...
- **Simulators don't replicate a device's performance or features.** Judge speed, memory and camera behavior only on a real device, ideally the oldest you support.
- **Most field crashes aren't in your clever code.** They're force-unwraps, launch-time stalls, memory terminations and other people's SDKs.
- **Launch time is a feature.** Nothing runs before the first frame that could run after it.
- **A denied permission usually stays denied.** The system prompt won't show again. Design the "no" path and link to Settings.
- **Old versions of your app live for years.** Your server has to keep serving them; version your API from day one.
- **Users skip versions.** Migrations must work from any old schema, not just the last one.
- **Long words and large text break more layouts than new screen sizes.** Test in German and at the largest accessibility size.
- **Pushes can arrive late, twice, or never.** Make each case harmless.
- **What works on your phone on the charger fails on theirs in Low Power Mode**, especially background work.
- **Most "SwiftUI is slow" is identity churn or work in `body`.** The SwiftUI instrument shows which views update and why.
- **Keep logic out of views and out of the app target.** A Swift package module builds and tests faster and can't reach the UI by accident.
...
- **Write App Review notes like documentation:** a demo account, what the AI features do, where data goes.
- **June's first beta is when you find out what breaks.** Waiting until September means shipping broken on update day.

## What's new in iOS 27 (and what old tutorials get wrong)

- **Scenes are mandatory.** Apps built with the iOS 27 SDK must use the scene-based lifecycle or fail to launch.
- **`@State` is a macro in Xcode 27**, and a class stored in it is initialized and stored only once.
- **Foundation Models opened up:** the `LanguageModel` protocol, `PrivateCloudComputeLanguageModel` (32K tokens, reasoning), `LanguageModelSession.DynamicProfile` for agent-style sessions, image input, and a new on-device model to retest against.
...
- **The app freezes at launch, then disappears; the report says `0x8badf00d`.** Cause: synchronous work on the main thread at launch (a migration, a network call, a model load). Fix: show the first frame, then do the work asynchronously; measure launch in Instruments.
...
- **After an update, synced data is missing on some devices.** Cause: a schema change without a migration, or a CloudKit change production won't accept. Fix: `VersionedSchema` from v1, additive-only CloudKit changes.
- **Bars look muddy under Liquid Glass.** Cause: custom bar backgrounds or glass in the content layer. Fix: remove them; keep glass for controls.
...
| `XCTestCase` for unit tests | Swift Testing; XCTest remains for UI tests |
...
| Custom blur behind bars (`UIVisualEffectView`) | Standard bars with Liquid Glass |
...
| View identity | How SwiftUI decides a view is "the same" across updates. |
| Source of truth | The single owner of a piece of state. |
| `@Observable` | Macro that lets a class report which properties were read and changed. |
| Liquid Glass | The iOS 26+ material for controls and navigation, floating over content. |
| Dynamic Type | The system text-size setting that text styles follow. |
| Size class | Compact or regular width and height; the basis of adaptive layout. |
| iPhone Duo | The foldable iPhone with outer and inner displays (September 2026). |
...
| Phased release | An update rolled out to automatic-update users over 7 days. |

## Practice

**1. Draw the map from memory.** On paper, draw today's map with its 10 parts and the arrows between them. Then compare.
*Done when:* you can explain every arrow in one sentence.

**2. Read an app like a platform engineer.** Pick an app you use daily. List every place it appears outside its main screen (widgets, Live Activities, notifications, Controls, actions in the Shortcuts app, Spotlight, the share sheet). Then list its permissions in Settings > Privacy & Security.
*Done when:* you have at least 8 rows, each naming the framework behind it.

**3. Route three AI features.** For (a) titling a note, (b) planning a two-week trip with 20 constraints, and (c) reading a menu from a photo, pick the model or framework (model 19, model 24).
*Done when:* each has a choice, a one-line reason, a fallback for when it's unavailable, and a consent sentence if data leaves Apple's models.

**4. Read the changelog.** Open [Updates](https://developer.apple.com/documentation/updates), then the SwiftUI and Foundation Models pages. Pick three changes the Errand capstone should use.
*Done when:* you have three bullets, each with an API name and the day it fits.

**5. Set up your machine: Xcode 27, a new iOS 27 project, running on your iPhone.** Each step follows the Apple page linked at its end.

1. Use an Apple silicon Mac on macOS Tahoe 26.6 or later and install Xcode 27 ([release notes](https://developer.apple.com/documentation/xcode-release-notes/xcode-27-release-notes)). Accept the iOS platform download, or get it later in Settings > Components ([components](https://developer.apple.com/documentation/xcode/downloading-and-installing-additional-xcode-components)).
2. Sign in under **Xcode > Settings > Apple Accounts**. A personal Apple Account can run apps on your own device ([running on devices](https://developer.apple.com/documentation/xcode/running-your-app-on-simulated-or-physical-devices)).
3. Choose **File > New > Project**, pick the iOS **App** template, name it `Errand`, choose your team and organization identifier, and select SwiftUI and Swift ([creating a project](https://developer.apple.com/documentation/xcode/creating-an-xcode-project-for-an-app)).
4. Select the target and set the minimum deployment to iOS 27.0 in the General pane. In **Build Settings**, check that Swift Language Version is Swift 6, Default Actor Isolation is MainActor, Approachable Concurrency is Yes, and Strict Concurrency Checking is Complete ([build settings](https://developer.apple.com/documentation/xcode/build-settings-reference)).
5. On the iPhone, turn on **Settings > Privacy & Security > Developer Mode**, restart, tap Enable and enter your passcode. The switch may appear only after you start pairing ([Developer Mode](https://developer.apple.com/documentation/xcode/enabling-developer-mode-on-a-device)).
6. Choose **Manage Devices…** from the run destination menu to open Device Hub. Click **+** and choose **Pair Nearby Device…** (iPhone on iOS 27, same Wi-Fi), or connect a cable. Tap **Trust** on the phone ([Device Hub](https://developer.apple.com/documentation/xcode/managing-your-simulated-and-physical-devices-in-device-hub)).
7. In **Signing & Capabilities**, turn on **Automatically manage signing**, pick your team, and use a unique bundle ID. Click **Register** if it appears ([running on devices](https://developer.apple.com/documentation/xcode/running-your-app-on-simulated-or-physical-devices)).
8. Choose your iPhone as the run destination and click **Run**.

*Done when:* the template's starter view appears on your iPhone, **View Screen** in Device Hub mirrors it, and the project builds with zero warnings in Swift 6 mode.

## Check yourself

**1.** Rows in your list lose their state every time the list refreshes. What's the likely cause?
<details><summary>Answer</summary>

Their identity changes on each update, usually because IDs are generated in `body` or derived from changing values. Store a stable `id` in the model and use it in `ForEach` (model 7).
</details>

**2.** Why can't an agent app keep a loop running in the background for an hour, and what do you do instead?
<details><summary>Answer</summary>

The system suspends background apps and grants time only through specific tools with limits (model 5), and guideline 2.5.4 bans abusing background modes. Run the loop on a server that pushes updates; use `BGContinuedProcessingTask` or `LongRunningIntent` for jobs the person starts, with visible progress.
</details>

**3.** A feature worked well on iOS 26 and got worse on iOS 27 with no code change. Why, and what would have caught it?
<details><summary>Answer</summary>

The on-device model is part of the OS and Apple updated it in iOS 27 (model 20). An evaluation set run on each OS beta would have shown the drop in June.
</details>
...
Only after clearly disclosing where it goes and getting explicit permission, per guideline 5.1.2(i) (models 2 and 19): a consent screen that names the provider, before the first request.
</details>

**6.** Where should an app store a login token, and why not `UserDefaults`?
<details><summary>Answer</summary>

In the Keychain, which is built and encrypted for secrets (model 27). `UserDefaults` is for small preferences.
</details>

**7.** With Default Actor Isolation set to MainActor, where does an unannotated function run, and how do you move CPU-heavy work off the main actor?
<details><summary>Answer</summary>

On the main actor (model 15). Mark the heavy function `@concurrent`, or put shared state in an `actor`, and `await` it, passing only `Sendable` values.
</details>

**8.** Name four places your app appears without its main UI running, and the framework that connects most of them.
<details><summary>Answer</summary>
...
- `State` and the `State()` macro — iOS 13.0 (macro form when building with Xcode 27)
- `Bindable` — iOS 17.0; `Binding` — iOS 13.0
- `Environment` — iOS 13.0; `Environment.init(_:)` for `Observable` types — iOS 17.0
- `Preview(_:body:)` macro (`#Preview`) — iOS 13.0
- `Observable()` macro — iOS 17.0
- `ScenePhase`, `EnvironmentValues.scenePhase` — iOS 14.0
- `App`, `WindowGroup` — iOS 14.0
- `List`, `Text`, `TextField`, `ForEach` — iOS 13.0
- `onSubmit(of:_:)` — iOS 15.0
- `id(_:)` — iOS 13.0
- `task(id:name:priority:file:line:_:)` — iOS 15.0
...
- `MainActor`, `Actor` — iOS 13.0
- `Sendable`, `Optional`, `Error` — iOS 8.0
- `Identifiable` — iOS 13.0; `UUID` — iOS 6.0
- Build settings `SWIFT_DEFAULT_ACTOR_ISOLATION`, `SWIFT_APPROACHABLE_CONCURRENCY`, `SWIFT_STRICT_CONCURRENCY`, `SWIFT_VERSION` — Xcode build settings reference (which also documents the `@concurrent` attribute)
- `Test(_:_:)`, `expect(_:_:sourceLocation:)` — Swift Testing (Xcode 16)
- `ProcessInfo.thermalState` — iOS 11.0; `ProcessInfo.isLowPowerModeEnabled` — iOS 9.0
...
- `AppIntent`, `AppIntent.perform()`, `AppEntity`, `AppEnum`, `AppShortcut` — iOS 16.0
- `IndexedEntity`, `AppIntent(schema:)` macro — iOS 18.0
- `IntentValueQuery`, `SnippetIntent` — iOS 26.0
- `LongRunningIntent`, `SyncableEntity` — iOS 27.0
- App Intents Testing framework — iOS 27.0
...
- `NSPersistentCloudKitContainer` — iOS 13.0
- `FileManager`, `UserDefaults` — iOS 2.0
- `URLSession` — iOS 7.0
- `MetricManager` — iOS 27.0; `MXMetricManager` — iOS 13.0 (marked deprecated in 27.2)
- Legacy, for recognition only: `ObservableObject`, `Published`, `EnvironmentObject` — iOS 13.0; `StateObject` — iOS 14.0; `NavigationView` — iOS 13.0 (marked deprecated in 27.2); `XCTestCase` — XCTest; `SFSpeechRecognizer` — iOS 10.0; `VNRecognizeTextRequest` — iOS 13.0; `INIntent` — iOS 10.0; `NSBundleResourceRequest` — iOS 9.0 (deprecated in 27.0); `MTLCommandQueue` — iOS 8.0; `UIVisualEffectView` — iOS 8.0
