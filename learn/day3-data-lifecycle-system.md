...
| `BGProcessingTask` | The system, can require power or network | Minutes | Maintenance, rebuilding indexes |
...
| `backgroundTask(_:action:)` | SwiftUI handler for app refresh and URL sessions | iOS 16 | [backgroundTask](https://developer.apple.com/documentation/swiftui/scene/backgroundtask(_:action:)) |
...
nonisolated enum PlacesAPI {
    ...
    static func hours(placeID: String) async throws -> PlaceHours {
        var request = URLRequest(url: URL(string: "https://api.example.com/v1/places/\(placeID)/hours")!)
...
- **Background task submission changed.** `BGTaskScheduler.submit(_:)` is deprecated in iOS 27. The replacement `submitTaskRequest(_:)` is async and shouldn't be called on the main thread. (This change is in the API reference, not the update notes, so older code and tutorials use `submit`.)
...
- **Document apps.** `ReadableDocument`, `WritableDocument`, and `URLDocumentConfiguration` read and write directly from file URLs. `FileDocument` and `ReferenceFileDocument` show as deprecated in 27.2.
...
- **MetricKit's `MetricManager`** delivers metric and diagnostic reports as async sequences and replaces `MXMetricManager`.
...
- **The widget shows nothing** → the extension opened a different store (not in the App Group), or a view had no container attached, in which case the environment gives you an in-memory, schema-less context where inserts throw and fetches return empty → use the same `ModelConfiguration(groupContainer:)` in every target and call `WidgetCenter.shared.reloadTimelines(ofKind:)` after the app saves.
...
| `FileDocument`, `ReferenceFileDocument` (deprecated 27.2) | `ReadableDocument`, `WritableDocument` |
| `ASCredentialUpdater` (deprecated 26.2) | `ASCredentialDataManager` |
| `MXMetricManager` and its subscriber | `MetricManager` (iOS 27) |
...
NSData.WritingOptions.completeFileProtection — iOS 2.0 (as reported for WritingOptions)<br>
ReadableDocument / WritableDocument / URLDocumentConfiguration — iOS 27.0<br>
FileDocument / ReferenceFileDocument — iOS 14.0, deprecated 27.2<br>
...
BGTaskScheduler.submitTaskRequest(_:) — iOS 27.0<br>
